...
export default function MessageDashboard({
  ...
  const adChatsCount = useMemo(() => {
    return chats.filter(c => !!(c.isFromAd || c.adInfo)).length;
  }, [chats]);
      ...
      <div className="sidebar" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          <ChatList 
            ...
            windowFilter={filter.kind === 'window' ? filter.value : (filter.kind === 'unread' ? 'unread' : (filter.kind === 'ad' ? 'ad' : null))}
            unreadFilter={filter.kind === 'unread'}
            adFilter={filter.kind === 'ad'}
